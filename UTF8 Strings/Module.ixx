export module UTF8_Strings:Module;
...
namespace utf8_string {
	...
	uint32_t getUTF8_SequenceLength(uint8_t byte)
	{
		...
		throw std::exception("Invalid byte format");
	}
	...
	// First 4 bits are the number of bytes in sequence
	// for single byte no ones are required 
	// for 2 ones 2 bytes are required
	// for 3 ones 3 bytes are required
	// for 4 ones 4 bytes are required
	//
	// each byte after the lead is prefixed with 10
	// 
	void encodeUTF8_CodePoint(uint32_t code_point,
	...
	utf8_string_iter::utf8_string_iter(utf8_string* p, int32_t i)
	{
		this->parent = p;
		this->byte_index = i;
	}

	void utf8_string_iter::operator++()
	{
		next();
	}

	void utf8_string_iter::operator++(int)
	{
		next();
	}
	...
	void utf8_string_iter::prev(uint32_t count)
	{
		if (isBeforeBegin()) {
			return;
		}
	...
	void utf8_string_iter::next(uint32_t count)
	{
		if (isAtNull()) {
			return;
		}
	...
	int32_t utf8_string_iter::characterIndex()
	{
		auto& bytes = parent->bytes;
	...
	uint32_t utf8_string_iter::codePoint()
	{
		auto& bytes = parent->bytes;
	...
	utf8_string utf8_string_iter::get()
	{
	...
	uint8_t* utf8_string_iter::data()
	{
		auto& bytes = parent->bytes;
		return bytes.data() + byte_index;
	}

	bool utf8_string_iter::isBeforeBegin()
	{
		return byte_index == -1;
	}

	bool utf8_string_iter::isNotAtNull()
	{
		return isAtNull() == false;
	}

	bool utf8_string_iter::isAtNull()
	{
		if (byte_index == -1) {
			return false;
		}
	...
	void utf8_string_iter::swap(utf8_string_iter& other)
	{
	...
	bool operator==(utf8_string_iter iter, const char8_t* str_literal)
	{
		utf8_string str = iter.get();
	...
	utf8_string::utf8_string(const char8_t* utf8_string_literal)
	{
		uint32_t count = 0;
	...
	utf8_string::utf8_string(std::string& string)
	{
	...
	utf8_string::utf8_string(std::vector<uint8_t>& new_bytes)
	{
		bytes.resize(new_bytes.size() + 1);
		std::memcpy(bytes.data(), new_bytes.data(), bytes.size());

		bytes[bytes.size() - 1] = '\0';
	}

	utf8_string utf8_string::operator[](uint32_t character_index)
	{
		iter i = begin();
		i.next(character_index);

		return i.get();
	}

	utf8_string utf8_string::at(uint32_t byte_index)
	{
		iter i = begin();
		i.byte_index = byte_index;
		return i.get();
	}

	uint8_t utf8_string::byte(uint32_t byte_index)
	{
		return this->bytes[byte_index];
	}

	uint32_t utf8_string::length()
	{
		if (isEmpty()) {
			return 0;
		}
	...
	bool utf8_string::isEmpty()
	{
		return bytes.size() == 1;
	}

	bool utf8_string::contains(const utf8_string& content)
	{
	...
	bool utf8_string::contains(const char8_t* utf8_string_literal)
	{
		utf8_string content = utf8_string_literal;
		return contains(content);
	}

	utf8_string_iter utf8_string::find(const utf8_string& content)
	{
		uint32_t content_idx = 0;
		uint32_t start = 0;
	...
	utf8_string_iter utf8_string::find(const char8_t* utf8_string_literal)
	{
		utf8_string content = utf8_string_literal;
		return find(content);
	}

	bool utf8_string::startsWith(const utf8_string& content)
	{
		for (uint32_t i = 0; i < content.bytes.size(); i++) {

			if (i >= this->bytes.size()) {
				return false;
			}

			uint8_t a = this->bytes[i];
			uint8_t b = content.bytes[i];
	...
	bool utf8_string::startsWith(const char8_t* utf8_string_literal)
	{
		utf8_string content = utf8_string_literal;
		return startsWith(content);
	}

	bool utf8_string::endsWith(const utf8_string& content)
	{
		auto a_index = this->bytes.size() - 1;
		auto b_index = content.bytes.size() - 1;

		for (uint32_t i = 0; i < content.bytes.size(); a_index--, b_index--, i++) {

			if (a_index < 0 || b_index < 0) {
				return false;
			}

			uint8_t a = this->bytes[a_index];
			uint8_t b = content.bytes[b_index];
	...
	bool utf8_string::endsWith(const char8_t* utf8_string_literal)
	{
		utf8_string content = utf8_string_literal;
		return endsWith(content);
	}

	utf8_string_iter utf8_string::before()
	{
		utf8_string_iter iter;
		iter.parent = this;
		iter.byte_index = -1;
	...
	utf8_string_iter utf8_string::begin()
	{
		utf8_string_iter iter;
		iter.parent = this;
		iter.byte_index = 0;
	...
	utf8_string_iter utf8_string::first()
	{
		return begin();
	}

	utf8_string_iter utf8_string::last()
	{
		utf8_string_iter iter;
		iter.parent = this;

		if (isEmpty() == false) {
			iter.byte_index = (uint32_t)bytes.size() - 1;
			iter.prev();
		}
	...
	utf8_string_iter utf8_string::after()
	{
		utf8_string_iter iter;
		iter.parent = this;
		iter.byte_index = (uint32_t)bytes.size() - 1;
	...
	utf8_string_iter utf8_string::end()
	{
		return after();
	}

	void utf8_string::push(uint32_t code_point)
	{
	...
	void utf8_string::push(utf8_string& new_content)
	{
		auto location = after();
		overwrite(location, new_content);
	}

	void utf8_string::push(const char8_t* utf8_string_literal)
	{
		utf8_string new_content = utf8_string_literal;
		push(new_content);
	}

	void utf8_string::prepend(utf8_string& new_content)
	{
		auto location = before();
		insert(location, new_content);
	}

	void utf8_string::prepend(const char8_t* utf8_string_literal)
	{
		auto location = before();
		utf8_string new_content = utf8_string_literal;
		insert(location, new_content);
	...
	void utf8_string::insert(iter& location,
	...
	void utf8_string::insert(iter& location, utf8_string& new_content)
	{
		iter new_content_start = new_content.begin();
		iter new_content_end = new_content.after();

		insert(location, new_content_start, new_content_end);
	}

	void utf8_string::insert(iter& location, const char8_t* utf8_string_literal)
	{
		utf8_string new_content = utf8_string_literal;
		insert(location, new_content);
	}

	void utf8_string::overwrite(iter& location,
		iter& new_content_start, iter& new_content_end)
	{
		if (new_content_start == new_content_end) {
			return;
	...
	void utf8_string::overwrite(iter& location, utf8_string& new_content)
	{
	...
	void utf8_string::overwrite(iter& location, const char8_t* utf8_string_literal)
	{
		utf8_string new_content = utf8_string_literal;
		overwrite(location, new_content);
	}

	void utf8_string::replaceSelection(iter& select_start, uint32_t select_length,
		utf8_string& new_content)
	{
		...
		uint32_t new_bytes_size = (uint32_t)bytes.size() - select_bytes_length + (uint32_t)new_content.bytes.size() - 1;
	...
	void utf8_string::replaceSelection(iter& selection_start, uint32_t selection_length,
	...
	void utf8_string::erase(iter& select_start, iter& select_end)
	{
	...
	void utf8_string::erase(iter& select_start, uint32_t selection_length)
	{
		iter select_end = select_start;
	...
	void utf8_string::erase(utf8_string& target)
	{
		...
		if (start != target.before()) {
			...
			iter end = start;
			end.byte_index += (uint32_t)target.bytes.size() - 1;

			erase(start, end);
		}
	}

	void utf8_string::erase(const char8_t* utf8_string_literal)
	{
		utf8_string target = utf8_string_literal;
		erase(target);
	}

	void utf8_string::pop(uint32_t chars_to_delete)
	{
		auto i = end();
	...
	void utf8_string::popFront(uint32_t chars_to_delete)
	{
		auto i = begin();
	...
	utf8_string utf8_string::copy(iter& begin, iter& end)
	{
		...
		utf8_string* other = begin.parent;
	...
	utf8_string utf8_string::copy(iter& begin, uint32_t count)
	{
	...
	bool operator==(const utf8_string& left, const char8_t* str_literal)
	{
