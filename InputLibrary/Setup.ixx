...
void setKeyState(WinInput& s, uint64_t wParam, bool is_down)
{
...
uint16_t getExtendedKey(WPARAM wParam, LPARAM lParam)
{
	bool is_rightside = (lParam & (1 << 24)) > 0;

	switch (wParam) {
	/*case VK_SHIFT: {
		uint32_t scan_code = (lParam >> 16) & 0xFF;
		if (scan_code == 42) return VirtualKeys::LEFT_SHIFT;
		else if (scan_code == 54) return VirtualKeys::RIGHT_SHIFT;
	}*/
	case VK_CONTROL: {
		return is_rightside == false ? VirtualKeys::LEFT_CONTROL : VirtualKeys::RIGHT_CONTROL;
	}
	case VK_MENU: {
		return is_rightside == false ? VirtualKeys::LEFT_ALT : VirtualKeys::RIGHT_ALT;
	}
	}

	return (uint16_t)wParam;
}

export namespace wininput {
	export void initWinInput(HWND hwnd, WinInput& s)
	{
		// Get Device List
	...
	export void startReadingInput(WinInput& s)
	{
		...
		s.wm_mousewheel_received = false;
	...
	export void endReadingInput(WinInput& s)
	{
		auto now = std::chrono::steady_clock::now();
	...
	export std::optional<int32_t> handleInputWindowMessages(WinInput& input, HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		switch (uMsg) {
			// Keyboard
		case WM_KEYDOWN: {
			setKeyState(input, getExtendedKey(wParam, lParam), true);
			return 0;
		}
		case WM_KEYUP: {
			setKeyState(input, getExtendedKey(wParam, lParam), false);
			return 0;
		}
		// F10
		case WM_SYSKEYDOWN: {
		...
		case WM_MOUSEWHEEL: {
			input.wm_mousewheel_received = true;
		...
		case WM_KILLFOCUS: {
			...
			// Mark all keys as UP
			for (auto& key : input.key_list) {

				auto* prev = &key.spans.back();

				// Extension
				if (prev->is_down == false) {
					// do nothing
				}
				// Transition
				else {
					auto& new_span = key.spans.emplace_back();
					new_span.is_down = false;
					new_span.transition = true;
					new_span.start_time = std::chrono::steady_clock::now();
				}
